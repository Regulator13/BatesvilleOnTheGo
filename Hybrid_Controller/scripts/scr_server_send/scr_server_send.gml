...
/// @function server_send_UDP(ip, index, buffer, length)
/// @description Server function to send a buffer to a client, individualized
/// @param Connected_client | player to send to
/// @param buffer | buffer with information to send
/// @param length | length of buffer already
//  Returns Message success
function server_send_UDP(Connected_client, buffer, length) {
	//ensure that player is the main one for the client, and not extended
	if not Connected_client.extended{
		...
		buffer_write(buffer, buffer_u8, Connected_client.udp_sequence_out)
		Connected_client.udp_sequence_out = scr_increment_in_bounds(Connected_client.udp_sequence_out, 1, 0, SEQUENCE_MAX, true)
	
		//send message
		Connected_client.message_success = network_send_udp(udp_server, Connected_client.ip, Connected_client.udp_port, buffer, length);
		return (Connected_client.message_success >= 0)
	}
